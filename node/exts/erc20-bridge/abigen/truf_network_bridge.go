...
//
// Solidity: event Withdraw(address indexed recipient, uint256 amount, bytes32 kwilBlockHash)
func (_TrufNetworkBridge *TrufNetworkBridgeFilterer) FilterWithdraw(opts *bind.FilterOpts, recipient []common.Address) (*TrufNetworkBridgeWithdrawIterator, error) {
	...
	logs, sub, err := _TrufNetworkBridge.contract.FilterLogs(opts, "Withdraw", recipientRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event Withdraw(address indexed recipient, uint256 amount, bytes32 kwilBlockHash)
func (_TrufNetworkBridge *TrufNetworkBridgeFilterer) WatchWithdraw(opts *bind.WatchOpts, sink chan<- *TrufNetworkBridgeWithdraw, recipient []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _TrufNetworkBridge.contract.WatchLogs(opts, "Withdraw", recipientRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
